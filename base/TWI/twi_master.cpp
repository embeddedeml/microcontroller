...
uint8_t twim_start (uint8_t Address, uint8_t TWIM_Type)
{
	 ...
	 */
	twst = TWSR & 0xF8;
	if ((twst != TWI_START) && (twst != TWI_REP_START)) return E_TWI_START;
	/*
	 ** Send device address
	 */
	 ...
	 */
	twst = TWSR & 0xF8;
	if ((twst != TWI_MTX_ADR_ACK) && (twst != TWI_MRX_ADR_ACK)) return E_TWI_START;
...
uint8_t twim_write (uint8_t byte)
{
	 ...
	 */
	twst = TWSR & 0xF8;
	if (twst != TWI_MTX_DATA_ACK) return E_TWI_WRITE;
